...
交易日历 + 当前美股时段
    |
    +-- 09:30 ET 前 ----> 生成/确认盘前 WatchCode -> 推荐监控（只提醒）
    |
...
项目只有一个全天自动入口：`monitor_auto.py`。它先通过统一交易日历判断阶段，再保证对应 WatchCode 已准备好，最后进入该阶段的监控；不会在每个入口各写一套日期或策略选择逻辑。
...
核心循环不再经过 `process_symbol` 或其他 `process_*` 编排包装。已有持仓由
`check_buy` 跳过并交给 `check_sell`；无持仓 WatchCode 在买入三阶段处理。默认
Broker 已在买卖执行阶段内部完成终态等待和自动超时撤单，服务层撤单三阶段不会
重复撤销 `CANCEL_REQUESTED` / `PENDING_CANCEL` 状态。
...
| 撤单 | `CancelStrategy` | 等待订单、超时撤单并确认最终状态 |
...
| --- | --- | --- | --- |
| 盘前，09:30 ET 前 | `data/watchcodes/watch_codes_premarket.txt` | 靠近动态 MA5 时发送推荐提醒 | 否 |
| 盘中，09:30-16:00 ET | `data/watchcodes/watch_codes.txt` | 运行买入、卖出和订单保护 | 仅买入窗口和风控允许时 |
