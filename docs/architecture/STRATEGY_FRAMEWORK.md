...
项目采用模块化单体，不拆微服务。WatchCode、买入、卖出和取消策略分别有独立契约与注册命名空间；`StrategyProfile` 只负责把四类策略和运行默认值组合成一个可复用名称。所有组件在外部行情、账户或订单 I/O 前一次性解析，非法配置失败关闭。自动盘中订单采用持久化跨轮监督；取消策略继续用于手动和兼容同步路径。

框架覆盖盘中 WatchCode、自动交易和通用回测主链路。盘前不参与选股框架，只读 Alpaca 当前持仓并对滚动 60 秒 3% 波动发送提醒；盘后各入口仍保留各自信号与下单边界，但已提交订单后的等待/撤单复用所选择的取消策略。历史专项回测如果有独立策略定义，不会被静默改成实盘策略。
...
| 分类 | 可选名称 |
| --- | --- |
| profile | `ma5_dip`、`ma5_dip_ladder`（盘中入口当前启用） |
| WatchCode | `ma5_dip` |
| 买入 | `ma5_dip` |
...
`ma5_dip_ladder` 复用 `ma5_dip` 的 WatchCode/首次买入信号及标准取消组件；三档建仓、基于加权平均成本的 `+10%` 首次半仓分档止盈、`-10%` MARKET 整仓止损和持久化进度属于 `service.py` + `ladder.py` 的有状态执行规则，不伪装成无状态 `BuyStrategy`/`SellStrategy`。分档状态和待确认订单状态都在 Broker I/O 前装载并校验；自动订单按订单 ID 的累计成交量幂等推进。
...
- WatchCode 生成和盘中监控共用同一个 `build_monitor_settings()`；生成器写入规范化规则头，所有盘中入口同时校验信号日和规则头，防止策略切换后误用旧池。
