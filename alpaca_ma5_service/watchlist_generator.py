...
def write_watch_codes(path: Path, candidates: list[WatchCandidate]) -> None:
    ...
    lines = [
        ...
        f"# Rules: gain>{MIN_SIGNAL_GAIN_PCT:.0%}, signal_gain>ma5_gain+{MIN_SIGNAL_GAIN_OVER_MA5_GAIN_PCT:.0%}, close/MA5>{MIN_CLOSE_TO_MA5_RATIO:g}, open/MA5>{MIN_OPEN_TO_MA5_RATIO:g}",
    ]
    if candidates:
...
def write_candidate_report(output_dir: Path, candidates: list[WatchCandidate]) -> None:
    ...
    with path.open("w", newline="", encoding="utf-8-sig") as f:
        writer = csv.DictWriter(
            f,
            fieldnames=["symbol", "signal_date", "gain_pct", "ma5_gain_pct", "body_pct", "upper_shadow_pct", "ma5", "ma10", "ma20", "open", "high", "close"],
        )
        writer.writeheader()
        for candidate in candidates:
