...
def run_afterhours_high_low_strategy(
    ...
    *,
    ...
    if is_regular_session(now_et):
        ...
        return []
    if not can_scan_after_regular_close(now_et):
        print_section("盘后策略待命")
        print_detail("运行时间", f"{now_et:%Y-%m-%d %H:%M:%S %Z}")
        print_detail("当前状态", "常规盘未收盘")
        print_detail("执行动作", "跳过盘后筛选")
        return []
    if not dry_run and not is_afterhours_buy_time(now_et):
        print_section("盘后策略待命")
        print_detail("运行时间", f"{now_et:%Y-%m-%d %H:%M:%S %Z}")
        print_detail("当前状态", "不在盘后买入窗口")
        print_detail("执行动作", "跳过真实挂单")
        return []
    ...
    if dry_run:
        ...
        print_step("4/4", "完成")
        print_detail("提示", "真实盘后挂单时，把 run_afterhours_buy.py 里的 AFTERHOURS_DRY_RUN 改成 False")
        return candidates
...
def write_afterhours_watch_codes(
    ...
    """写出盘后策略专用观察池。"""
    lines = [
        "# Auto-generated by run_afterhours_buy.py",
